...
#ifndef EXEC_NODE_V1_H
#define EXEC_NODE_V1_H
...
class ExecNodeV1 : public ExecNode {
    
    public:
        ExecNodeV1(ObjStore * obj_store, ExecNode* child_node, const Predicate & pred, bool is_last, bool base_only, unsigned int buf_size, uint64_t vid, uint32_t level, Type type, Transaction::ReadSet * read_set);
        ~ExecNodeV1();
        bool next() override;
    ...
    private:
        ...
        Type type_;
        TransactionV1::ReadSet * read_set_;
        // if node is of oid type, the lower_bound_ and upper_bound_ is set based on the objet id
        // false by default
        bool oid_;
